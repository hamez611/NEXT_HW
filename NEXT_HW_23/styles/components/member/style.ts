...
export const MemberImgBox = styled.div`
    width: 100%; // 부모 요소의 100% 너비로 제한
    max-width: 220px; // 최대 너비 설정 (필요에 따라 조정)
    max-height: 220px; // 최대 높이 설정 (필요에 따라 조정)
    overflow: hidden; // 넘치는 이미지가 잘리도록 설정
    display: flex;
    justify-content: center;
    align-items: center;

    img {
        width: 100%; // 이미지가 부모의 너비에 맞춰지도록 설정
        height: auto; // 높이는 비율에 맞춰 자동으로 설정
